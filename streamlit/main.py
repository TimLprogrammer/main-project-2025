...
if menu == "Latest Predictions":
    ...
    if st.session_state['run_clicked']:
        try:
            # Check if forecast predictions file exists
            forecast_file = "../predictions/forecast_predictions.csv"
            if not os.path.exists(forecast_file):
                st.warning("No forecast predictions found. Please run the forecast pipeline first.")
                st.stop()
            ...
            # Check if model metrics files exist
            cooling_metrics_file = "../predictions/best-model/cooling/best_model_metrics.json"
            heating_metrics_file = "../predictions/best-model/heating/best_model_metrics.json"

            if os.path.exists(cooling_metrics_file):
                with open(cooling_metrics_file) as f:
                    cooling_metrics = json.load(f)
            else:
                cooling_metrics = {
                    "model_name": "No model trained yet",
                    "timestamp": "N/A",
                    "metrics": {"R²": 0.0, "MSE": 0.0, "RMSE": 0.0, "MAE": 0.0}
                }

            if os.path.exists(heating_metrics_file):
                with open(heating_metrics_file) as f:
    ...
    if st.button("Show Current Model"):
        ...
        try:
            ...
            if os.path.exists(cooling_metrics_file):
                ...
                st.warning("No cooling model metrics found. Please train models first.")
                all_metrics = {}
        except Exception as e:
...
"""
        ...
        for model in model_names:
            ...
            for filename, explanation in plot_types.items():
                plot_path = os.path.join(base_path, filename)
                with st.expander(f"{model} - {filename}"):
                    if os.path.exists(plot_path):
                        st.image(plot_path, use_container_width=True)
                        st.write(explanation)
                    else:
                        st.warning(f"{filename} not found for {model} model.")
