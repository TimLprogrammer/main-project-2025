...
def train_ensemble_model(X_train, y_train, X_test, y_test, lgbm_params=None, gb_params=None, xgb_params=None):
    ...
    # Select only the best models for the ensemble
    best_models = []
    for name, model in models.items():
        if base_metrics[name]['R²'] > 0.4:  # Only models with R² > 0.4 (increased threshold)
    ...
    # Use KFold instead of TimeSeriesSplit for StackingRegressor
    # because TimeSeriesSplit doesn't work with cross_val_predict
    ...
    return None, models, base_predictions, base_metrics
...
def save_best_model(models, metrics, save_path):
    ...
    # Save model
    model_path = os.path.join(save_path, f'best_model_cooling.pkl')
    with open(model_path, 'wb') as f:
        pickle.dump(best_model, f)

    # Also save all models for ensemble use
    all_models_path = os.path.join(save_path, 'all_models_cooling.pkl')
...
def add_temporal_features(df):
    """Add advanced time-dependent features to the dataset"""
...
def main():
    ...
    try:
        data_path = path.join(BASE_PATH, 'data', 'csv-daily', 'cooling', 'cooling_daily.csv')
        ...
        try:
            alt_data_path = path.join(BASE_PATH, 'data', 'cooling_daily.csv')
    ...
    # For cooling data we use the random split that performed better earlier
    # but we keep the option for chronological split available
    use_time_split = False  # Set to True to use chronological split
    ...
    # Feature scaling - Use StandardScaler as in the original version
    ...
    try:
        # Use fewer trials (5) to make the optimization faster
        gb_params = optimize_gradient_boosting(X_train_scaled, y_train, n_trials=5)
    except Exception as e:
        logger.error(f"Error during GradientBoosting optimization: {e}")
        # Use predefined parameters that work well for cooling data
        gb_params = {
            'n_estimators': 500,
            'learning_rate': 0.05,
            'max_depth': 6,
    ...
    plots_path = path.join(BASE_PATH, 'predictions', 'plots', 'cooling')
    ...
    best_models_path = path.join(BASE_PATH, 'predictions', 'best-model', 'cooling')
...
if __name__ == "__main__":
    ...
    logger = logging.getLogger(__name__)
    logger.info("Starting cooling model training and evaluation")
    ...
    logger.info("Cooling model training and evaluation completed")
