...
def process_fault_data(notifications_file, system_type):
    ...
    try:
        ...
        # Convert date column
        system_df['Created on'] = pd.to_datetime(system_df['Gecreëerd op'])
        system_df['date'] = system_df['Created on'].dt.date
...
def create_fault_boxplot(daily_data, system_name, output_dir):
    ...
    try:
        ...
        outliers = daily_data[z_scores > 6]  # Increased from 4 to 6
        ...
        # Remove outliers
        daily_data = daily_data[z_scores <= 6]  # Increased from 4 to 6
        return daily_data
    except Exception as e:
